...
Файлы для этого задания можно найти в репозитории в папке \href{https://mipt-hsse.gitlab.yandexcloud.net/v.biryukov/devtools_course/-/tree/main/seminar7_bash_scripting/practice/files}{\texttt{seminar7\_bash\_scripting/practice/files}}.
...
В этой задаче будет нужно подменить команду \texttt{cat} так, чтобы она вместо того, чтобы печатать содержимое файла/файлов, печатала на экран одно слово \texttt{"Meow"}, вне зависимости от переданных ей аргументов. Добейтесь подобного поведения, изменив переменную \texttt{PATH}. После выполнения задания, перезагрузите терминал, чтобы вернуть прежнее значение переменной \texttt{PATH}.
...
Напишите программу, которая будет печатать первый переданные ей аргумент.
\begin{lstlisting}
$ ./script.sh
...
\noindent Чему равны коды возврата следующий команд. Проверьте себя с помощью \texttt{echo \$?}
\begin{multicols}{2}
\begin{enumerate}[(a)]
\item \texttt{[[ \$a == apple ]]}
\item \texttt{[[ \$a == apple ]]}
...
\begin{itemize}
\item \texttt{Less} -- если первая строка лексикографический меньше второй.
\item \texttt{Equal} -- если строки равны.
\item \texttt{Greater} -- если первая строка лексикографический больше второй.
\end{itemize}
...
\item С использованием \texttt{if} и программы \texttt{ls}.
\item С использованием оператора \texttt{||} и программы \texttt{ls} (без использования \texttt{if}).
\end{enumerate}
...
\begin{itemize}
\item \texttt{Does not exist} -- если файла не существует.
...
Напишите программу, которая будет печатать все переданные ей аргументы в следующем формате:
\begin{lstlisting}
$ ./script.sh
...
$ cd space_test
$ ../script.sh *
Argument 1: alpaca.txt
Argument 2: bison.txt
Argument 3: camel carries goods.txt
\end{lstlisting}
...
\subsection{Раличие между группой команд и сабшеллом}
...
echo $a
\end{lstlisting}
Для решения этой задачи создайте текстовый файл, содержащий вывод скрипта.\\
...
Напишите программу, которая принимает одну строку, через аргумент и печатает следующее:
\begin{enumerate}[(a)]
...
Напишите программу, которая будет принимать строку, являющуюся веб-адресом, через аргумент и печатать домен верхнего уровня этого веб-адреса.
\begin{lstlisting}
...
\begin{tabular}{ l | l }
 ...
 \texttt{-{}-prefix n} &  возвращает только первые \texttt{n} символов строки\\
\end{tabular}
\end{flushleft}

\noindent При работы такой программы:
\begin{lstlisting}
...
\subsection{Куб}
Напишите программу, которая принимает числа через аргумент и печатает куб этого числа.
\begin{lstlisting}
$ ./script.sh 5
125
\end{lstlisting}
...
map["fox"]="fig"
\end{lstlisting}
Произведите с словарём следующие операции:
\begin{enumerate}[(a)]
...
\item Добавьте в словарь пару с ключом \texttt{"dog"}.
\end{enumerate}
\section{Функции}


\subsection{Факториал}
Напишите программу, содержащую функцию \texttt{fact}. Эта функция должна принимать число через аргумент и возвращать факториал этого числа через \texttt{echo}. Если на вход приходит отрицательное число, то код возврата функции должен быть \texttt{1}, в ином случае код возврата должен быть \texttt{0}.
...
\noindent Функция должна иметь код возврата \texttt{1}, если файла не существует и \texttt{0} в ином случае.
...
Если какого-то файла из переданных не существует, то функция для этого файла должна вывести в \texttt{stderr}:
\begin{lstlisting}
add_suffix: cannot acces <filename>: No such file or directory
\end{lstlisting}
При это все остальные файлы должны быть обработаны. Функция должна возвращать код возврата \texttt{0}, если все файлы существуют и они были переименованы, и \texttt{1} в иных случах.
...
Напишите программу, которая будет принимать через аргумент названия файла, а затем печатать этот файл на экран, добавляя номер строки и количество символов в строке перед началом каждой строки.
