...
\begin{enumerate}
\item Пустая команда ":"
\item Присвоить переменной значение, если оно не присвоено:
\begin{lstlisting}
: "${a:=apple}"
\end{lstlisting}

\item Задача на массивы + проверку строки на пустоту.
\item В 17 добавить проверку на Failed to delete file
\item ls как проверка на существование, доработать
\item \texttt{"\$\{!i\}"}
\end{enumerate}
