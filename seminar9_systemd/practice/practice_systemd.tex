...
\item \texttt{WantedBy} -- эта строка говорит, к какому таргету будет подключаться данный сервис при вызове \texttt{systemctl enable}. \texttt{multi-user.target} -- это таргет, который исполняется при загрузке системы. Соответственно, эта настройка означает, что сервис \texttt{alpaca} будет запускаться при запуске системы (если его автозапуск включен через \texttt{systemctl enable}).
\end{itemize}
...
Также создайте файл \texttt{s0\thesubsection.sh}, который бы представлял собой скрипт, запускаемый данным сервисом.

\subsection{Сервис логов по таймеру}
Создайте сервис который бы записывал в файл \texttt{/var/log/mylog2.log} текущую дату и время, загрузку процессора и и количество занятой оперативной памяти каждые две минуты. В этой задаче не используйте \texttt{sleep}. Вместо этого используйте systemd timers.
...
Также создайте файл \texttt{s0\thesubsection a.sh}, который бы представлял собой скрипт, запускаемый данным сервисом.
...
Также создайте файл \texttt{s0\thesubsection b.sh}, который бы представлял собой скрипт, запускаемый данным сервисом.
\end{enumerate}
