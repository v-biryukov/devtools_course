...
...
\end{lstlisting}
Для каждой подзадачи нужно прописать все команды, которые исполняются в ходе выполнения этой подзадачи. Если в задаче встречается вопрос, то на этот вопрос нужно ответить в комментариях (начинаются с \texttt{\#}) скрипта. Если в задаче требуется в определённый момент перейти в другой терминал, отметьте это в файле решения, используя комментарий:
\begin{lstlisting}
# Переходим во второй терминал
...
# Возвращаемся в первый терминал
\end{lstlisting}
Если в задаче требуется написать скрипт, то вместо файла решения нужно создать один скрипт с расширением \texttt{.sh} и шебангом в начале скрипта. Названия файлов решений для всех задач должны начинаться с номера задачи, например, \texttt{01.sh}, даже если в условии задачи используется другое имя для скрипта.
...
\end{lstlisting}
Напишите или скачайте этот скрипт из репозитория и дайте ему права на исполнение.
...
$ ./counter.sh > a.txt
\end{lstlisting}
В результате в \texttt{a.txt} будут записываться числа. Bash будет занят записью чисел в файл, поэтому в данной оболочке мы ничего сделать не сможем. Но можно проверить, что числа печатаются в файл, используя другой терминал. Откройте другой терминал, зайдите в ту же директорию и проверьте, что в файл \texttt{a.txt} в данный момент производится запись, несколько раз вызвав \texttt{cat}. Также можно использовать команду:
\begin{lstlisting}
$ watch -n 2 cat a.txt
\end{lstlisting}
...
$ ./counter.sh > a.txt &
\end{lstlisting}
Выполните команду \texttt{ps}, которая покажет все программы, запущенные в данном терминале. Скорее всего на экран выведется информация о процессах \texttt{bash} (оболочка), \texttt{counter.sh}, \texttt{sleep} (запущенный внутри \texttt{counter.sh}) и только что запущенный \texttt{ps}. Для каждого процесса будет выведен его идентификатор PID (\textit{process identifier}). Используйте полученный PID процесса \texttt{counter.sh} и команду \texttt{kill}, чтобы завершить процесс.
Команда \texttt{kill <PID>} отправляет сигнал \texttt{SIGTERM} процессу с идентификатором \texttt{<PID>}, что приводит к его завершению. 
Проверьте, что процесс завершился и запись в \texttt{a.txt} больше не производится. Ещё раз выполните команду \texttt{ps} и убедитесь, что процесс \texttt{counter.sh} отсутствует в выводе.
\end{enumerate}
...
\item Перейдите на основной терминал. Нажмите комбинацию клавиш \texttt{Ctrl-Z}, чтобы приостановить процесс. Теперь процесс приостановился и не будет ничего записывать в файл. Убедитесь в этом на втором терминале. 
...
\item Снова приостановите процесс, используя \texttt{Ctrl-Z}. Используйте команду \texttt{bg}, чтобы возобновить процесс. Теперь он будет работать в фоновом режиме. Убедитесь, что процесс начал работать, посмотрев вывод на другом терминале.
...
\end{lstlisting}
Так как эта задача является последней добавленной, можно использовать \texttt{bg} без указания номера. В этом случае операция будет применена к последней добавленной задаче. Снова посмотрите на все задачи, используя \texttt{jobs}.

\item Используйте команду
\begin{lstlisting}
$ kill %N   # вместо N нужно подставить номер задачи в jobs
\end{lstlisting}
чтобы завершить задачу, записывающую данные в файл \texttt{c.txt}. Снова посмотрите на все задачи.

\item Используйте команду
\begin{lstlisting}
$ kill -STOP %N   # вместо N нужно подставить номер задачи в jobs
\end{lstlisting}
чтобы остановить задачу, записывающую данные в файл \texttt{b.txt}, послав сигнал \texttt{SIGSTOP}. Посмотрите на все задачи. Используйте \texttt{bg} или 
\begin{lstlisting}
$ kill -CONT %N   # вместо N нужно подставить номер задачи в jobs
\end{lstlisting}
чтобы возобновить приостановленную задачу, послав сигнал \texttt{SIGCONT}.
\end{itemize}
\end{enumerate}
...
\item Напечатайте все процессы, используя \texttt{ps -ef}. Эта команда покажет информацию о uid, pid, ppid, c, stime, tty, time, cmd всех процессов в системе. Найдите среди этих процессов процесс \texttt{counter.sh}. Чему равны PID и PPID для этого процесса?
...
\item Напечатайте все процессы, показав информацию только о pid, ppid, pcpu, pmem, comm.
\item Напечатайте все процессы, показав только pid, ppid, pcpu, pmem, comm и отсортировав по полю pmem.
\item Используйте \texttt{ps} с опцией \texttt{-p}, чтобы напечатать информацию uid, pid, ppid, pcpu, pmem, comm только для процесса \texttt{counter.sh}.
\item Используйте \texttt{ps} вместе с командой \texttt{grep}, чтобы напечатать информацию uid, pid, ppid, pcpu, pmem, comm только для \texttt{counter.sh}.
...
\item Используйте команду \texttt{kill}, чтобы остановить процесс \texttt{counter.sh} на другом терминале. Убедитесь, что процесс остановлен и находится в списке \texttt{jobs} соответствующего терминала. Ещё раз используйте \texttt{kill}, чтобы возобновить работу скрипта \texttt{counter.sh}.
...
Напишите скрипт, который будет делать следующее:
\begin{enumerate}
...
\item Печатать на экран PID родителя данного процесса, а также команду с помощью которой был запущен родитель (используйте \texttt{ps -o cmd}).
\item Печатать на экран PID самого процесса, а также команду, с помощью которой он был запущен.
\item Печатать на экран PID ребёнка данного процесса, а также команду, с помощью которой он был запущен.
\end{enumerate}

\subsection{Родословная}
Напишите скрипт, который будет принимать PID процесса через аргумент и выводить рекурсивно PID его родителей и команды, с помощью которых были запущены все процессы. Последний родитель будет иметь имя \texttt{systemd} (или \texttt{/sbin/init}) и иметь PID равный 1.
...
Создайте скрипт \texttt{create\_group.sh}, использующий скрипт \texttt{counter.sh} из предыдущих задач (он должен находиться в той же директории) и содержащий:
\begin{lstlisting}
#!/bin/bash
./counter.sh &
./counter.sh &
...
\end{lstlisting}
чтобы распечатать информацию о PID, PPID и PGID (\textit{process group id}) процессов, а также имя команды, запустившей процесс. Какие процессы были созданы после запуска скрипта \texttt{create\_group.sh}? Убедитесь, что все созданные процессы принадлежат одной группе процессов. Какой процесс является лидером этой группы процессов?
\item Завершите процесс скрипта \texttt{create\_group.sh} по его PID, послав ему сигнал с помощью команды \texttt{kill}. Завершились ли при этом дочерние процессы скрипта? Если дочерние процессы не завершились, то какой процесс является их родителем? Если дочерние процессы не завершились, то завершите их с помощью \texttt{pkill}.
\item Заново запустите скрипт \texttt{create\_group.sh}. Используйте \texttt{ps} чтобы узнать PGID новой группы процессов. Завершите сразу все процессы одной группы, послав им всем сигналы. Используйте только один вызов команды \texttt{kill}.
...
\end{lstlisting}
чтобы распечатать информацию о PID, PPID, PGID и SID (\textit{session id}) процесса, а также имя команды, запустившей процесс. Убедитесь, что значения PGID одинаковы для процессов, находящихся в одной группе. Убедитесь, что значения SID одинаковы для всех созданных процессов. Кто является лидером сессии?
...
sleep 1000
\end{lstlisting}
Если запустить этот скрипт и нажать \texttt{Ctrl-C}, то завершится ли скрипт? Почему у скрипта такое поведение в этом случае?


\item \textbf{Игнорирование сигнала}
...
$ ./counter.sh > c.txt
\end{lstlisting}
то после закрытия терминала процессу \texttt{counter.sh} будет послан сигнал \texttt{SIGHUP} и он завершится, так как этот процесс не перехватывает и не игнорирует сигналы.
Используя команду \texttt{nohup}, запустите скрипт \texttt{counter.sh} с игнорированием сигнала \texttt{SIGHUP}. После этого закройте терминал, откройте новый терминал и убедитесь, что процесс \texttt{counter.sh} всё ещё жив. Завершите этот процесс с помощью \texttt{kill}.



\subsection{Сигналы от дочерних процессов}
Есть скрипт \texttt{random\_sleeper.sh}, который просто спит случайное число секунд от 1 до 10:
\begin{lstlisting}
#!/bin/bash
sleep $((RANDOM % 10 + 1))
\end{lstlisting}

\noindent Напишите скрипт \texttt{waiter.sh}, который будет принимать число \texttt{n} через аргументы командной строки и запускать скрипт \texttt{random\_sleeper.sh} параллельно \texttt{n} раз. Скрипт \texttt{waiter.sh} должен дожидаться всех своих дочерних процессов, используя команду \texttt{wait}. Также скрипт должен отслеживать завершения всех дочерних процессов с помощью \texttt{trap}. После того как любой из дочерних процессов завершится, \texttt{waiter.sh} должен сразу печатать на экран строку вида:
\begin{lstlisting}
...
Напишите скрипт, который создаёт спящий процесс и остановленный процесс. После этого скрипт должен напечатать PID этих процессов. Два созданных процесса должны существовать после того, как скрипт завершится. Используйте \texttt{ps aux}, чтобы просмотреть состояния процессов.
...
\item \texttt{I<} (для потока ядра)
\end{enumerate}
Для решения этой задачи создайте файл \texttt{\thesubsection.txt} с ответами на все вопросы.
...
\end{lstlisting}
Системный вызов \texttt{fork} порождает дочерний процесс, клонируя родительский. Можно считать, что в \texttt{fork} заходит один процесс (родительский), а из \texttt{fork} выходят два процесса (родительский и дочерний). В дочернем процессе \texttt{fork} возвращает \texttt{0}. В родительском -- значение PID ребёнка. В данной программе после \texttt{fork} родительский процесс засыпает на 1000 секунд, а дочерний процесс сразу пытается завершиться с помощью \texttt{exit}. Обычно, родительский процесс должен прочитать код возврата дочернего процесса, но он этого не делает и в результате дочерний процесс становится зомби процессом.
...
Запомните PID родительского процесса и PID дочернего зомби процесса.
...
\end{enumerate}
\end{multicols}
\noindent Для решения этой задачи создайте файл \texttt{\thesubsection.txt} с ответами на все вопросы.


\subsection{Директория \texttt{/proc}}
\begin{itemize}
\item Запустите программу \texttt{counter.sh} и найдите PID процесса.
