...
1		& SIGHUP	& Завершение 			 & Посылается оболочке и процессам в \texttt{jobs} при закрытии терминала.\\ \hline
2		& SIGINT	& Завершение 			 & Посылается передним процессам, при нажатии \texttt{Ctrl+C}.\\
...
9		& SIGKILL	& Завершение без		 & Принудительное завершение, не дающее программе  \\
		& &			  возможности перехвата  & возможности сохранить данные или корректно освободить  \\
										 & & & ресурсы. Не может быть перехвачен процессом.\\ \hline
15		& SIGTERM	& Завершение 			 & Стандартный запрос на корректное завершение процесса.\\
										 & & & Используется по умолчанию, если не указать сигнал в \texttt{kill}\\ \hline
...
19		& SIGSTOP	& Приостановка без 		 & Принудительно приостанавливает выполнение процесса.\\
		& & 		  возможности перехвата	 & Не может быть проигнорирован или перехвачен процессом \\ \hline
...
\subsection*{Разновидности команды \texttt{trap}}

\begin{itemize}
\item \texttt{trap \textquotesingle команда\textquotesingle ~сигнал}

Перехватывает сигнал и выполняет команду. Например, если мы напишем в скрипте:
\begin{lstlisting}
trap 'echo Hello' TERM
\end{lstlisting}
то, если процессу этого скрипта послать сигнал \texttt{SIGTERM}, вместо завершения будет выполнена команда \texttt{echo Hello}. Данное поведение не наследуется дочерними процессами. Если послать сигнал дочернему процессу, то он завершится, даже если у родительского процесса переопределён обработчик сигнала.

\item \texttt{trap \textquotesingle\textquotesingle ~сигнал}

Блокирует поступающий сигнал. Например, если мы напишем в скрипте:
\begin{lstlisting}
trap '' TERM
\end{lstlisting}
то, если процессу этого скрипта послать сигнал \texttt{SIGTERM}, он не будет реагировать на данный сигнал.
Данное поведение наследуется дочерними процессами. Если послать сигнал дочернему процессу, то он не завершится, потому что он унаследует это блокирование сигнала у родительского процесса.

\item \texttt{trap \textquotesingle команда\textquotesingle ~EXIT}

Выполняет команду при выходе из скрипта. Команда выполнится при любом завершении скрипта кроме:
\begin{itemize}
\item Завершении программы с помощью сигнала \texttt{SIGKILL}.
\item Аварийном завершении программы.
\end{itemize} 


\item \texttt{trap \textquotesingle команда\textquotesingle ~ERR}

Выполняет указанную команду при возникновении ошибки в любой команде скрипта. То есть если какая-нибудь команда в скрипте завершится с ненулевым кодом возврата. За исключением команд, используемый в \texttt{if}, \texttt{while} и операторах \texttt{\&\&} и \texttt{||}.
\end{itemize}
\newpage
~
\newpage
