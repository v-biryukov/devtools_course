...
Что такое аргументы командной строки? Как запустить скрипт, передав ему некоторые аргументы?
Переменные \texttt{\$1}, \texttt{\$2}, \texttt{\$0}, \texttt{\$*}, \texttt{\$@}. Чем отличается \texttt{"\$*"} от \texttt{"\$@"}? Что выведет \texttt{echo "hello \$0"}, если запустить эту команду в терминале и почему?
...
Директория \texttt{/proc} и файлы в этой директории:
\vspace{-3mm}
\begin{multicols}{2}
\begin{itemize}
\item \texttt{cmdline}
