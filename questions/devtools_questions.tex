...
Где находится таблица MBR и сколько место занимает? Что содержится внутри MBR? Ограничение на количество разделов и размер каждого раздела. Типы разделов: первичный (primary), расширенный (extended), логический (logical).
...
\end{multicols}
\vspace{-3mm}
Запуск команд \texttt{parted} в скриптовом и тихом режиме. 
...
\begin{itemize}
\item Что будет, если удалить исходный файл, на который указывает мягкая ссылка?
\item Можно ли создать мягкую ссылку на директорию?
\item Можно ли создать мягкую ссылку на файл, находящийся в другой файловой системе?
\end{itemize}
...
\item Переходит ли \texttt{find} по симлинкам на директории? Опция \texttt{-L} команды \texttt{find}.
\end{itemize}
...
ext4, xfc, FAT, NTFS. Виртуальные файловые системы. Особенности каждой из этих файловых систем.
...
Aргументы командной строки. Как запустить скрипт, передав ему некоторые аргументы?
...
\item \textbf{Группировка команд и сабшеллы}
...
\item Zombie
\end{itemize}
В каких случаях процесс попадает в то или иное состояние? Процессы в каких состояниях нельзя завершить, послав им сигналы, в том числе сигнал SIGKILL?

\item \textbf{Управление заданиями}\\
Фоновый и передний режим. Как запустить процесс в фоновом режиме? Команды \texttt{jobs}, \texttt{fg}, \texttt{bg}. Использование \texttt{kill} для того, чтобы посылать сигналы процессам из \texttt{jobs}. Как остановить и возобновить процесс? Какой сигнал и каким процессам посылается при:
\begin{itemize}
...
Команда \texttt{top}, что она показывает? Что означают значения \texttt{load average}?\\
Что означают поля процентов занятого времени процессора (\texttt{\%CPU(s)}):
\vspace{-3mm}
\begin{multicols}{3}
\begin{itemize}
\item \texttt{us}
